...
namespace NPC.Scripts.Items
{
    public class Sapper : Item
    {
        ...
        public override void Use(Character character)
        {
            switch (Trapped)
            {
                ...
                default:
                    ...
                    Collider2D[] playerColliders = Physics2D.OverlapCircleAll(mousePosition, sapperEffectRadius, _playerMask);
                    foreach (Collider2D playerCollider in playerColliders)
                    {
                        Player player = playerCollider.GetComponent<Player>();
                        if (player != null && Vector2.Distance(player.transform.position, mousePosition) <= sapperEffectRadius)
                        {
        ...
        private void ParticleEffect(Vector2 position)
        {
            ...
            mainModule.duration = sapperEffectDuration;
